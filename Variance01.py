...
def get_valid_float(prompt: str) -> float:
    """Ensures the user enters a valid numeric value (integer or decimal)."""
    while True:
        try:
...
def show_data_summary(data: dict[Any, Any]) -> None:
    """Displays a clean summary panel of all currently entered group data."""
    print("\n📊 --- Current Data Review Panel ---")
    print("-" * 55)
    for group, info in data.items():
        print(
            f"  {group} -> Freq(N): {info['frequency']:<6} "
            f"Mean: {info['mean']:<6} "
            f"Variance: {info['variance']:<6}"
        )
    print("-" * 55)


def get_data() -> dict[Any, Any]:
    """Handles structured data entry and mid-stream data corrections."""
    data = dict()
    group_number = 1
    ...
    while True:
        ...
        # Validation loop for the quit status prompt
        while True:
    ...
    while True:
        ...
        # If user presses Enter without typing anything, proceed to calculation
        if not modify_choice:
            break

        target_group = f"Group {modify_choice}"

        # Dynamic check and local overwrite if the group exists
...
# Initialize data gathering with confirmation checks
Data = get_data()

# 1. Compute the global population mean (x_bar)
...
# 2. Compute the total sum of squares using the simplified double-sum formula
